...
const { getNativeToken } = require("../helpers/configHelper");
const {
  addNativeToken,
...
const deployStorage = async () => {
  ...
  storeContractAddresses([
    { network: network, contractName: "Storage", address: storage.target },
  ]);
  return storage;
};
...
const deployTokenFactory = async () => {
  ...
  console.log(
    `🔵 Deploying TokenFactory with Storage address ${storageAddress} ...`
  );
...
const deployVault = async () => {
  ...
  console.log(
    `🔵 Deploying Vault contract with Storage address ${storageAddress} ...`
  );
  ...
  storeContractAddresses([
    { network: network, contractName: "Vault", address: vault.target },
  ]);
  return vault;
};
...
const deployRelayerBase = async () => {
  ...
  console.log(
    `🔵 Deploying RelayerBase contract with Storage address ${storageAddress} ...`
  );
  const relayerBase = await hre.ethers.deployContract("RelayerBase", [
    storageAddress,
  ]);
...
const deployBridgeBase = async () => {
  ...
  console.log(
    `🔵 Deploying BridgeBase contract with Storage ${storageAddress} and RelayerBase ${relayerAddress} addresses ...`
  );
  const bridgeBase = await hre.ethers.deployContract("BridgeBase", [
    storageAddress,
    relayerAddress,
  ]);
...
const updateOperator = async () => {
  ...
  for (const contract in contractAddresses.contracts) {
    if (operators.hasOwnProperty(contract)) {
      ...
      console.log(
        `🔵 Setting ${operatorName} with address ${contractAddress} ...`
      );
...
const addEth = async (networkEth) => {
  ...
  } else if (network === "allfeat" || network === "allfeat_local") {
